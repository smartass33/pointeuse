		...
		<g:javascript>
			function ajaxCall(){
				if (document.getElementById("siteSelect") != null && document.getElementById("siteSelect") !== undefined && document.getElementById("periodSelect") != null && document.getElementById("periodSelect") !== undefined){
					...
					$(function(){$('#details').load('${createLink(controller:'site', action:'getAjaxSiteData',onLoading:'document.getElementById("spinner").style.display = "inline";',onComplete:'document.getElementById("spinner").style.display = "none";',params:[year:year,month:month])}&site='+site+'&period='+period);});
				}
			}
		</g:javascript>
	...
	<body>
		...
		<div id="site-followup" class="standardNav">
			<h1>	
				...
				<g:else><g:message code="sites.followup" /></g:else>
				<br>
				<br>
				<g:form method="POST" url="[controller:'site', action:'siteTotalTime']">
					...
					<li>	
						<g:actionSubmit class='pdfButton' value="PDF"  action="completeSiteReportPDF"/>				
					</li>			
					<g:if test="${site != null}">	
