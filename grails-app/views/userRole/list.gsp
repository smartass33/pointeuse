			...
			<table>
				...
				<g:each in="${userRoleInstanceList}" status="i" var="userRoleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${userRoleInstance.id}">${userRoleInstance?.role?.authority}</g:link></td>
					
						<td>${userRoleInstance?.user?.firstName} ${userRoleInstance?.user?.lastName}</td>
