		...
		<div id="show-authorizationType" class="content scaffold-show" role="main">
			...
			<ol class="property-list authorizationType">		
				...
				<li class="fieldcontain">
					...
					<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${authorizationTypeInstance?.user?.id}">${authorizationTypeInstance?.user?.firstName} ${authorizationTypeInstance?.user?.lastName}</g:link></span>
				</li>
				</g:if>	
			</ol>
